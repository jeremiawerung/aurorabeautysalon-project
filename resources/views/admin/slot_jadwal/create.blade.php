...
@section('title', 'Tambah Slot Jadwal - Aurora')
...
<div class="content-wrapper">
    <div class="container-fluid">
        ...
        <form id="form-slot-jadwal-create" action="{{ route('slot-jadwal.store') }}" method="POST">
            @csrf
            ...
            <div class="card mb-4">
                <div class="card-header d-flex flex-column flex-md-row justify-content-md-between align-items-start align-items-md-center">
                    <h5 class="mb-2 mb-md-0">
                        <i class="bi bi-clock-history me-2"></i>Tambah Slot Jadwal
                    </h5>
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        <label for="id_layanan" class="form-label">
                            <i class="bi bi-list-ul me-1"></i>Pilih Layanan
                        </label>
                        ...
                        <div class="info-box" id="info-box" style="display: none;">
                            <small id="durasi-info">
                                <i class="bi bi-clock me-1"></i><strong>Durasi:</strong> <span id="durasi-value">-</span> menit
                            </small>
                            <small id="slot-info">
                                <i class="bi bi-calendar-check me-1"></i><strong>Slot tersedia:</strong> <span id="slot-value">-</span>
                            </small>
                        </div>
                    </div>
                    ...
                    <div class="empty-state" id="empty-state">
                        ...
                        <p>Silakan pilih layanan terlebih dahulu untuk menambahkan slot jadwal</p>
                    </div>

                    <!-- Daftar Slot -->
                    <div class="slot-container" id="slot-section">
                        ...
                        <div id="slot-container">
                            <!-- Slot items will be added here dynamically -->
                        </div>
                        ...
                        <div class="alert alert-info d-flex align-items-start" style="font-size: 0.8rem; margin-top: 1rem;">
                            ...
                            <div>
                                <strong>Perhatian:</strong> Tidak boleh ada slot dengan waktu yang sama. 
                                Jarak antar slot minimal sama dengan durasi layanan untuk mencegah bentrokan jadwal.
                            </div>
                        </div>
                    </div>
                </div>
...
@php
    $slotMap = [];
    foreach ($slotByLayanan as $idL => $collection) {
        $slotMap[$idL] = $collection->map(function ($s) {
            return [
                'waktu'       => \Carbon\Carbon::parse($s->waktu)->format('H:i'),
                'status_slot' => $s->status_slot,
            ];
        })->values();
    }
    ...
    $slotTextMap = [];
    foreach ($slotByLayanan as $idL => $collection) {
        $slotTextMap[$idL] = $collection
            ->sortBy('waktu')
            ->map(fn($s) => \Carbon\Carbon::parse($s->waktu)->format('H:i'))
            ->implode(', ');
    }
@endphp
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const slotsByLayanan   = @json($slotMap);
    const slotTextByLayanan = @json($slotTextMap);
    ...
    function updateDurasiInfoAndSlots() {
        ...
        const idLayanan = opt ? opt.value : null;
        const slotText = idLayanan && slotTextByLayanan[idLayanan]
            ? slotTextByLayanan[idLayanan]
            : '';
        
        slotValue.textContent = slotText || 'Belum ada slot';

        loadSlotsForSelectedLayanan();
    }

    function createSlotRow(waktu = '', status = 'aktif') {
        ...
        const statusSelect = row.querySelector('select[name="status_slot[]"]');

        if (waktu) waktuInput.value = waktu;
    ...
    function loadSlotsForSelectedLayanan() {
        const idLayanan = layananSelect.value;
        const slots = slotsByLayanan[idLayanan] || [];

        slotContainer.innerHTML = '';

        if (slots.length > 0) {
            slots.forEach(s => {
                const row = createSlotRow(s.waktu, s.status_slot || 'aktif');
    ...
    addBtn.addEventListener('click', function () {
        ...
        if (inputs.length > 0) {
            ...
            if (lastVal && durasiMenit > 0) {
                defaultTime = addMinutesToTime(lastVal, durasiMenit);
            }
        }
        const row = createSlotRow(defaultTime);
        slotContainer.appendChild(row);
        refreshRemoveButtonsVisibility();
    });
