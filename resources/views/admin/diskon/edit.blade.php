...
@section('title', 'Edit Diskon - Aplikasi')
...
<style>
    ...
    .status-toggle-group input[type="radio"]#status_kedaluwarsa:checked + .status-btn {
        ...
        <form action="{{ route('diskon.update', $diskon->id) }}" method="POST">
            ...
            <div class="card mb-4">
                ...
                <div class="card-header">
                    <h5 class="mb-0">Form Edit Diskon: {{ $diskon->kode_diskon }}</h5>
                </div>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-5">
                            ...
                            <input 
                                ...
                                required>
                            <small class="form-text text-muted">Pastikan Kode Diskon tetap unik.</small>
                            @error('kode_diskon')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select 
                                ...
                                @foreach($layanans as $layanan)
                                    <option value="{{ $layanan->id_layanan }}"
                                        {{ (is_array(old('layanan_ids')) ? in_array($layanan->id_layanan, old('layanan_ids')) : in_array($layanan->id_layanan, $layananTerpilih)) ? 'selected' : '' }}
                                    >
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input 
                                ...
                                id="tanggal_mulai" 
                                name="tanggal_mulai" 
                                value="{{ old('tanggal_mulai', $diskon->tanggal_mulai ? \Carbon\Carbon::parse($diskon->tanggal_mulai)->format('Y-m-d') : '') }}" 
                                required>
                            @error('tanggal_mulai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="status-toggle-group">
                                <input type="radio" name="status_diskon" id="status_aktif" value="aktif" 
                                    {{ old('status_diskon', $diskon->status_diskon) == 'aktif' ? 'checked' : '' }}>
                                <label for="status_aktif" class="status-btn">Aktif</label>
                                
                                <input type="radio" name="status_diskon" id="status_tidak_aktif" value="tidak aktif" 
                                    {{ old('status_diskon', $diskon->status_diskon) == 'tidak aktif' ? 'checked' : '' }}>
                                <label for="status_tidak_aktif" class="status-btn">Tidak Aktif</label>
                                
                                <input type="radio" name="status_diskon" id="status_kedaluwarsa" value="kedaluwarsa" 
                                    {{ old('status_diskon', $diskon->status_diskon) == 'kedaluwarsa' ? 'checked' : '' }}>
                                <label for="status_kedaluwarsa" class="status-btn">Kedaluwarsa</label>
                            </div>
                            @error('status_diskon')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input 
                                ...
                                value="{{ old('tanggal_berakhir', $diskon->tanggal_berakhir ? \Carbon\Carbon::parse($diskon->tanggal_berakhir)->format('Y-m-d') : '') }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea 
                                ...
                                name="keterangan"
                                rows="3"
                                placeholder="Penjelasan singkat tentang ketentuan diskon...">{{ old('keterangan', $diskon->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card-footer d-flex justify-content-start">
                    <button type="submit" class="btn btn-simpan-pink">Update Diskon</button>
